...
public interface CandidateOfferRepository extends JpaRepository<CandidateOfferEntity, UUID> {
    ...
    // Drives the Offer Approvals screen: server-side candidate-name search + status/position-filter,
    // paginated, newest first. allowedStatuses is the fixed set of statuses visible at that approval
    // level; status further narrows within that set when the approver picks a specific status filter.
    @Query("SELECT o FROM CandidateOfferEntity o WHERE o.status IN :allowedStatuses AND " +
            "(:status IS NULL OR o.status = :status) AND " +
            "(:positionId IS NULL OR EXISTS (SELECT 1 FROM CandidateEntity c WHERE c.id = o.candidateId AND c.positionId = :positionId)) AND " +
